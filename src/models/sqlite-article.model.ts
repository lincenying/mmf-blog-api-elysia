...
export class ArticleModel {
    public static getAll(page: number = 1, pageSize: number = 10) {
        ...
        const data = db.query<Article, null>(`SELECT * FROM article order by id desc LIMIT ${pageSize} OFFSET ${offset}`).all(null)
        const total = db.prepare(`SELECT COUNT(*) as total FROM article`).get() as { total: number }
